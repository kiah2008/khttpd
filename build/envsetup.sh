...
function gettop
{
    local TOPFILE=build/envsetup.mk
...
function mm()
{
    ...
    # normal make.
    if [ -f build/envsetup.mk -a -f Makefile ]; then
        $DRV make $@
    else
        ...
        if [ ! "$T" ]; then
            ...
            ONE_SHOT_MAKEFILE=$M $DRV make -C $T -f build/main.mk $MODULES $ARGS
        fi
    fi
}
...
function mmm()
{
    ...
    if [ "$T" ]; then
        ...
        ONE_SHOT_MAKEFILE="$MAKEFILE" $DRV make -C $T -f build/main.mk $DASH_ARGS $MODULES $ARGS
    else
        echo "Couldn't locate the top of the tree.  Try setting TOP."
        return 1
    fi
...
function mma()
{
  ...
  if [ -f build/envsetup.mk -a -f Makefile ]; then
    ...
    $DRV make -C $T -f build/main.mk $@ $MODULES_IN_PATHS
  fi
}
...
function mmma()
{
  ...
  if [ "$T" ]; then
    ...
    # Convert "/" to "-".
    MODULES_IN_PATHS=${MODULES_IN_PATHS//\//-}
    $DRV make -C $T -f build/main.mk $DASH_ARGS $ARGS $MODULES_IN_PATHS
  else
    echo "Couldn't locate the top of the tree.  Try setting TOP."
    return 1
  fi
